...
const fetchContractEmployees = async () => {
  try {
    const response = await axios.get(`${API_BASE}/api/employees/contract`);
...
export default function ContractEmployeesList() {
  ...
  useEffect(() => {
    ...
    const loadEmployees = async () => {
      const data = await fetchContractEmployees();
      setEmployees(data);
    };
  ...
  const handleSave = async () => {
    ...
    const updatedList = await fetchContractEmployees();
  ...
  return (
    ...
    <div className="contract-container">
      ...
      <div className="contract-content">
        ...
        <div className="contract-employees-container">

          <h2>Contract Employees List</h2>
          ...
          <div className="contract-employees-grid">

            {filteredEmployees.length > 0 ? (

              filteredEmployees.map((employee) => (

                <div key={employee.id} className="contract-employee-card">

                  {employee.image ? (
                    <img
                      src={`${API_BASE}/uploads/${employee.image}`}
                      alt={employee.name}
                      className="contract-employee-image"
                    />
                  ) : (
                    <div className="contract-employee-image" style={{ display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#e0e0e0", color: "#888", fontSize: "14px" }}>
                      No Image
                    </div>
                  )}

                  <div className="contract-employee-details">
