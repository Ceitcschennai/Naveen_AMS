...
import "../styles/InternshipEmp.css";
...
const fetchInternshipEmployees = async () => {
  try {
    const response = await axios.get(`${API_BASE}/api/employees/internship`);
...
export default function InternshipEmployees() {
  ...
  useEffect(() => {
    ...
    const loadEmployees = async () => {
      const data = await fetchInternshipEmployees();
      setEmployees(data);
    };
  ...
  const handleSave = async () => {
    ...
    const updatedList = await fetchInternshipEmployees();
  ...
  return (
    ...
    <div className="internship-container">
      ...
      <div className="internship-content">
        ...
        <div className="internship-employees-container">

          <h2>Internship Employees List</h2>
          ...
          <div className="internship-employees-grid">

            {filteredEmployees.length > 0 ? (

              filteredEmployees.map((employee) => (

                <div key={employee.id} className="internship-employee-card">

                  {employee.image ? (
                    <img
                      src={`${API_BASE}/uploads/${employee.image}`}
                      alt={employee.name}
                      className="internship-employee-image"
                    />
                  ) : (
                    <div className="internship-employee-image" style={{ display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#e0e0e0", color: "#888", fontSize: "14px" }}>
                      No Image
                    </div>
                  )}

                  <div className="internship-employee-details">
